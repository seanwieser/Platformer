...
class Player(object):
    ...
    def drawInventoryGuns(self, win):
        i = 0
        for gunName in self.gunManager.getGunNames():
            win.blit(WEAPONS_INV[gunName], INV_POS[i])
            ammoText = str(self.gunManager.gunAmmo(gunName)[0])+'/'+str(self.gunManager.gunAmmo(gunName)[1])
            ammoSurface = UNI_FONT.render(ammoText, 1, (255,255,255), INV_BG_COLOR)
            xText = INV_POS[i][0] + WEAPON_DIM[0]//2 - UNI_FONT.size(ammoText)[0]//2
            win.blit(ammoSurface, (xText, INV_POS[i][1]+INV_WIN_HEIGHT-30))
            i+=1
    ...
    def curSprite(self):
        ...
        if self.hasGun:
            return answer+self.gunManager.equipped().getName()
        return answer
    ...
    def updateShootAbility(self):
        if self.hasGun and self.gunManager.clipAmmo() > 0:
            self.canShoot = True
        else:
            self.canShoot = False
    ...
    def toggleGunNum(self, newGunIndex):
        if newGunIndex >= 0:
            ...
            if self.gunManager.getGunNum() == len(self.gunManager.gunAmount())-1:
...
class GunManager(object):
    ...
    def clipAmmo(self):
        return self.guns[self.gunNum].getAmmo()[0]
    ...
    def equippedAmmo(self):
        return self.guns[self.gunNum].getAmmo()
    def gunAmmo(self, gunName):
        return self.guns[self.getGunIndex(gunName)].getAmmo()
...
class Gun(object):
    def __init__(self, gunName, ammo):
